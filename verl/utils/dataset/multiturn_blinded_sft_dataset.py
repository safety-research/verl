...
class MultiTurnBlindedSFTDataset(Dataset):
    """
    ...
    def _read_files_and_process(self):
        def series_to_item(ls):
            ...
            for l in ls:
                if 'tool_calls' in l and l['tool_calls'] is None:
                    del l['tool_calls']
                elif 'tool_calls' in l and l['tool_calls'] is not None:
                    l['tool_calls'] = list(l['tool_calls'])

                if 'tool_call_id' in l and l['tool_call_id'] is None:
                    del l['tool_call_id']
    ...
    def __getitem__(self, item):
        ...
        try:
            full_tokens = tokenizer.apply_chat_template(messages, tokenize=True, return_tensors="pt", add_generation_prompt=False, tools=input_gating_tool_config)
        ...
        for i, msg in enumerate(messages):
            ...
            prefix_tokens = tokenizer.apply_chat_template(prefix_messages, tokenize=True, return_tensors="pt", add_generation_prompt=False, tools=input_gating_tool_config)

            # Get tokens for messages up to previous point
            prev_tokens = tokenizer.apply_chat_template(messages[:i], tokenize=True, return_tensors="pt", add_generation_prompt=False, tools=input_gating_tool_config) if i > 0 else None
